...
def train(source, target, lamb1,lamb2):
    ...
    if (yaml_data['transfer_loss_fir'] == "mmd"):
        transfer_loss_fir=MMDLoss().to(device) #MMD域适应损失
    if (yaml_data['transfer_loss_sec'] == "lmmd"):
        transfer_loss_sec = LMMDLoss(num_class=int(yaml_data['num_class'])).to(device)  # MMD域适应损失
    ...
    for i in range(epoch):
        ...
        total_transfer_loss_fir = 0  # 整体迁移Loss
        total_transfer_loss_sec = 0  # 整体迁移Loss
        #训练步骤开始
        ...
        for data in train_dataloader:
            ...
            loss2 = transfer_loss_fir(source_fc_data, target_fc_data)

            target_logits=torch.nn.functional.softmax(target_outputs, dim=1)
            loss3 = transfer_loss_sec(source_fc_data.to(device), target_fc_data.to(device),targets,target_logits)
            # print('loss2:', "%2.20f"%loss2)
            loss = loss1 + lamb1 * loss2+ lamb2 * loss3
            #优化器调优
            ...
            total_transfer_loss_fir = total_transfer_loss_fir + loss2.item()
            total_transfer_loss_sec = total_transfer_loss_sec + loss3.item()
            total_train_step=total_train_step+1
            if total_train_step%50==0:
                print("总训练批次: {},【整体】Loss: {}, 【分类】Loss: {}, 【迁移1】Loss: {}, 【迁移2】Loss: {}".format(total_train_step,loss.item(),
        ...
        print("第{}训练后的【训练集-迁移1】Loss为: {}".format(i + 1, total_transfer_loss_fir))
        print("第{}训练后的【训练集-迁移2】Loss为: {}".format(i + 1, total_transfer_loss_sec))
        #一轮训练后，进行测试
        wang.eval()
    ...
    for i in yaml_data['groups']:
        for j in yaml_data['groups']:
            if i != j:  # 排除重复组合
                ...
                for k in np.linspace(yaml_data['lambda_range_fir'][0], yaml_data['lambda_range_fir'][1], yaml_data['lambda_range_fir'][2]):

                    # combination = (i, j)
                    # print(yaml_data['lambda_range'][0])
                    # print(source,'-',target)
                    # print(k)
                    for v in np.linspace(yaml_data['lambda_range_sec'][0], yaml_data['lambda_range_sec'][1],
                                         yaml_data['lambda_range_sec'][2]):
                        train(source,target,k,v)
