...
class YoloBody(nn.Module):
    def __init__(self, num_classes, backbone="densenet121", pretrained=False):
...
#     wang=YoloBody() #实例化网络模型
#     wang=wang.to(device) #将模型转移到cuda上
#     input=torch.ones((64,3,512,512)) #生成一个batchsize为64的，1个通道的10X10 tensor矩阵-可以用来检查网络输出大小
#     input=input.to(device) #将数据转移到cuda上
#     output=wang(input) #将输入喂入网络中进行处理
#     print(output.shape)
#     summary(wang,input_size=(3,512,512)) #输入一个通道为1的10X10数据，并展示出网络模型结构和参数
