...
from .vgg import vgg
from .attention import se_block, cbam_block, eca_block, CA_Block  #注意力机制
...
class YoloBody(nn.Module):
    def __init__(self, num_classes, backbone="densenet121", pretrained=False, phi = 0):
        ...
        self.phi            =phi
        if phi >=1 and phi <=4:
            self.x0_attiontion=attention_blocks[phi-1](512)
            self.x1_attiontion=attention_blocks[phi-1](512)
            self.x2_attiontion=attention_blocks[phi-1](256)
        ...
        self.conv_for_end = conv2d(512, 1, 1)
    ...
    def forward(self, x):
        ...
        if self.phi >= 1 and self.phi <= 4:
            x2 = self.x2_attiontion(x2)
            x1 = self.x1_attiontion(x1)
